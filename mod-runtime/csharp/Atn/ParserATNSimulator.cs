	...
	public class ParserATNSimulator : ATNSimulator
	{
											 ...
											 ParserRuleContext outerContext)
		{
			...
			{ // while more work
			  ...
			  //							   " line "+input.LT(1)Line+":"+input.LT(1).Column);
				...
				if (debug)
				{
					System.Console.WriteLine("Reach " + reach);
				}
				if (reach == null)
				{
					// if any configs in previous dipped into outer context, that
				...
				if (debug)
				{
					Console.WriteLine(reach.ToString());
					Console.WriteLine("LL altSubSets=" + String.Join(", ", altSubSets) +
				...
				var xx = GetUniqueAlt(reach);
				//System.Console.WriteLine("d = " + xx);
				reach.uniqueAlt = xx;
				// unique prediction?
				...
				if (mode != PredictionMode.LL_EXACT_AMBIG_DETECTION)
				{
					...
					//System.Console.WriteLine("a " + predictedAlt);
					if (predictedAlt != ATN.INVALID_ALT_NUMBER)
					{
						break;
					}
										  ...
										  bool complete)
		{
			...
			foreach (PredPrediction pair in predPredictions)
			{
				...
				if (debug || dfa_debug)
				{
					Console.WriteLine("eval pred " + pair + "=" + (predicateEvaluationResult ? "true" : "false"));
				}
								...
								bool treatEofAsEpsilon)
		{
			...
			//			if (debug) { jjjj++; Console.WriteLine("closure_() " + jjjj); }
			// optimization
										...
										bool fullCtx)
		{
			if (debug)
			{
				Console.WriteLine("PRED (collectPredicates=" + (collectPredicates ? "true" : "false") + ") " +
						pt.precedence + ">=_p" +
						", ctx dependent=true");
				if (parser != null)
				{
